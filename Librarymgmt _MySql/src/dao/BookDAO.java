...
public class BookDAO {
    ...
    public static List<Book> getBooksByTitle(String title) {
        ...
        String query = "SELECT * FROM Lib_books WHERE LOWER(title) LIKE LOWER(?)"; // Case-insensitive search
    
        // Database connection parameters
        String url = "jdbc:mysql://localhost:3307/librarydb";  // Your actual database name
        ...
        try (Connection conn = DriverManager.getConnection(url, username, password);
            ...
            pst.setString(1, "%" + title + "%");  // Add wildcard for partial matching
            try (ResultSet rs = pst.executeQuery()) {
                while (rs.next()) {
                    Book book = new Book(
                        rs.getString("book_id"),  // Use 'book_id' as the column name from the database
                        rs.getString("title"),
                        rs.getString("author"),
                        rs.getString("publisher"),
                        rs.getInt("quantity")
